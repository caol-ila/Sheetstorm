...
pub fn detect_measure_bars(
    ...
    for (idx, sys) in systems.iter().enumerate() {
        ...
        // Bar-Run muss Y-Range nahezu exakt = [top_y..bot_y] sein.
        // Toleranz: ±0.4*spacing Margin (Stems gehen weiter raus).
        let margin = (spacing * 0.4) as u32;
        let min_dist = (spacing * 4.0) as i64;

        // Vor-filter: NH-X-Positionen für dieses System.
        ...
        while x < bin.w {
            ...
            // 1) Spalte muss zwischen top_y..bot_y vollständig schwarz sein
            if !column_is_full_between(bin, x, top_y, bot_y) {
                x += 1;
                continue;
            }

            // 2) Bar-Run-Breite messen
            let mut x_end = x;
            while x_end + 1 < bin.w {
                let ty = *top_line.y_per_x.get((x_end + 1) as usize).unwrap_or(&0);
                let by = *bot_line.y_per_x.get((x_end + 1) as usize).unwrap_or(&bin.h);
                if !column_is_full_between(bin, x_end + 1, ty, by) { break; }
                x_end += 1;
            }
            let bar_width = x_end - x + 1;

            // 3) Filter: nicht zu dick (sonst ist es ein dickes Symbol)
            if bar_width > max_thickness {
                x = x_end + 1;
                continue;
            }

            // 4) Y-Range-Check: Run darf NICHT deutlich über/unter den Stafflinien rausragen
            //    (= Stem-Filter). Bestimme TOP+BOT des realen schwarzen Runs in der Spalte.
            let bar_x = (x + x_end) / 2;
            ...
            if extends_above || extends_below {
                // Sieht aus wie ein Stem mit Notehead darüber/darunter
                x = x_end + 1;
                continue;
            }

            // 5) Notehead-Proximity-Check: kein Bar wenn Notehead innerhalb 0.7*spacing
            ...
            // 6) Min-Distance zum vorherigen Bar
            if (bar_x as i64) - last_bar_x < min_dist {
                x = x_end + 1;
                continue;
            }

            // 7) Linke und rechte Nachbarspalte müssen WEISS sein (isolierte Linie)
            //    Wir prüfen 2px links und 2px rechts vom Bar-Run.
            if !flanks_are_white(bin, x, x_end, top_y, bot_y) {
                x = x_end + 1;
                continue;
            }
...
/// Spalte zwischen [top_y, bot_y] muss zu mindestens 80% schwarz sein,
/// max. 3 aufeinanderfolgende weiße Pixel.
/// (Nach 90°-Auto-Rotation entstehen oft Anti-Aliasing-Artefakte; vorher
/// 0.85/max=2 war zu strikt für gescannte/rotierte Vorlagen.)
fn column_is_full_between(bin: &Binary, x: u32, top_y: u32, bot_y: u32) -> bool {
...
/// Wir prüfen mehrere Y-Positionen ZWISCHEN den Stafflinien (nicht auf den Linien
/// selbst, weil dort die Stafflinie quer durchgeht und sowieso schwarz ist).
fn flanks_are_white(bin: &Binary, bar_x_start: u32, bar_x_end: u32, top_y: u32, bot_y: u32) -> bool {
    let bot = bot_y.min(bin.h.saturating_sub(1));
    if top_y >= bot { return false; }
    let h = bot - top_y;
    let spacing = h / 4; // 4 Räume zwischen 5 Linien
