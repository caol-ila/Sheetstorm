...
def discover_classes(root: Path) -> Tuple[List[str], Dict[str, int]]:
    """Findet alle Leaf-Klassen in root.

    Erwartet Struktur: root/category/class/*.png
    Gibt (sorted class names, class_to_idx) zurück.
    """
    classes = []
...
def load_corpus(
    ...
    """Lädt alle Samples aus root, optional mit max_per_class Limit."""
...
class TripletDataset(Dataset):
    """Liefert (anchor, positive, negative) Triplets aus synthetic_corpus_v1.

    Pro Epoch werden n_triplets zufällige Triplets generiert:
      - Anchor:   zufälliges Sample einer zufälligen Klasse
      - Positive: anderes Sample der gleichen Klasse
      - Negative: Sample einer anderen zufälligen Klasse

    Rufe _regenerate_triplets() am Anfang jeder Epoch auf.
    """
    ...
    def _regenerate_triplets(self):
        """Generiert neue zufällige Triplets (einmal pro Epoch aufrufen)."""
        triplets = []
...
class EmbeddingModel(nn.Module):
    """MobileNetV3-Small + projection-head → embedding_dim-dim L2-normalized.

    Architektur:
      MobileNetV3-Small backbone (ohne Classifier)
...
def triplet_loss(
    ...
    cosine_distance(a, b) = 1 - dot(a, b)  (bei L2-normierten Vektoren)
...
def build_transforms(train: bool) -> transforms.Compose:
    """ImageNet-Normalisierung + optionale Augmentierung."""
...
def verify_onnx(model: nn.Module, onnx_path: Path) -> float:
    ...
    try:
        ...
        print("  (onnxruntime nicht installiert, Verifikation übersprungen)")
        return float("nan")
...
def train(args) -> Tuple[nn.Module, Path, float]:
    """Triplet-Training-Pipeline. Gibt (model, onnx_path, best_val_top1) zurück."""
    ...
    # Sicherstellen dass .pt existiert (auch wenn top1=0 in allen Epochs)
...
def main():
    ...
    parser.add_argument("--corpus", default="data/synthetic_corpus_v1/single",
                        help="Pfad zum Corpus-Verzeichnis (category/class/*.png)")
    parser.add_argument("--output", default="models/symbol_encoder_v1",
                        help="Output-Praefix (.pt/.onnx/.json/.log)")
