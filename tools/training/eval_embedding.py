...
Output:
    Top-1 / Top-3 / Top-5 Accuracy, Per-Class-Metrics, Confusion-Matrix
"""
...
def embed_all(
    ...
    """Bettet alle Dateien ein. Gibt (embeddings [N, D], labels [N]) zurueck."""
...
def build_confusion_matrix(labels, preds, n_classes):
...
def per_class_metrics(cm, class_names):
    ...
    for i, name in enumerate(class_names):
        ...
        metrics.append({"class": name, "precision": round(precision, 4),
                        "recall": round(recall, 4), "f1": round(f1, 4), "support": int(cm[i].sum())})
    return metrics
...
def main():
    ...
    all_files = [(f, class_to_idx[cls]) for cls, files in class_to_files.items() for f in files]
    ...
    if args.output:
        ...
        Path(str(out) + ".metrics.json").write_text(
            json.dumps({"knn": knn_results, "macro_f1": macro_f1, "per_class": per_class}, indent=2),
            encoding="utf-8")
