...
/// Helper: builds a ProviderContainer with a SpielmodusNotifier for [notenId]
/// and waits for the initial async load to complete.
Future<(ProviderContainer, SpielmodusNotifier)> _makeNotifier(
  String notenId,
) async {
  ...
  final sub = container.listen(spielmodusProvider(notenId), (_, __) {});
  addTearDown(sub.close);

  final notifier = container.read(spielmodusProvider(notenId).notifier);
  // Wait for _loadSheetMusic (100 ms delay in implementation)
  await Future<void>.delayed(const Duration(milliseconds: 200));
  return (container, notifier);
}

SpielmodusState _state(ProviderContainer c, String id) =>
...
void main() {
  ...
  group('SpielmodusNotifier — initial load', () {
    ...
    test('loads 8 pages and selects default stimme', () async {
      ...
      expect(state.currentStimmeId, 'kl2');
      expect(state.stimmen, hasLength(7));
    });
  ...
  group('SpielmodusNotifier — singlePage navigation', () {
  ...
  group('SpielmodusNotifier — halfPageTurn mode (AC-13..AC-20)', () {
  ...
  group('SpielmodusNotifier — twoPage mode', () {
  ...
  group('SpielmodusNotifier — overlay', () {
  ...
  group('SpielmodusNotifier — UI lock', () {
  ...
  group('SpielmodusNotifier — view mode for orientation', () {
  ...
  group('SpielmodusNotifier — stimme change', () {
    test('changeStimme updates currentStimmeId and resets to page 0', () async {
      final (container, notifier) = await _makeNotifier('st-1');
      notifier.goToPage(3);
      notifier.changeStimme('tr1');
      final state = _state(container, 'st-1');
      expect(state.currentStimmeId, 'tr1');
      expect(state.currentPage, 0);
    });

    test('changeStimme hides overlay (AC-42 / AC-40)', () async {
      final (container, notifier) = await _makeNotifier('st-2');
      notifier.toggleOverlay();
      notifier.changeStimme('kl1');
  ...
  group('SpielmodusNotifier — setlist navigation', () {
    final setlist = [
      const SetlistItem(stueckId: 's1', title: 'Stück 1', orderIndex: 0),
      const SetlistItem(stueckId: 's2', title: 'Stück 2', orderIndex: 1),
      const SetlistItem(stueckId: 's3', title: 'Stück 3', orderIndex: 2),
    ];
  ...
  group('SpielmodusNotifier — auto-scroll speed calculation', () {
  ...
  group('SpielmodusNotifier — zoom memory per page', () {
