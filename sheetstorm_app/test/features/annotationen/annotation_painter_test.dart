...
import 'package:sheetstorm/features/annotationen/presentation/widgets/stamp_picker.dart';
...
void main() {
  ...
  group('AnnotationPainter — gespeicherte Annotationen rendern', () {
    ...
    test('paint() läuft fehlerfrei mit Stift-Annotation', () {
      final painter = AnnotationPainter(
        annotations: [_annot(level: AnnotationLevel.privat)],
        layerVisibility: const LayerVisibility(),
      );
      expect(() => _paintOnCanvas(painter), returnsNormally);
    });
    ...
    test('paint() mit allen 3 Ebenen gleichzeitig', () {
      final painter = AnnotationPainter(
        annotations: [
          _annot(id: 'p', level: AnnotationLevel.privat),
          _annot(id: 's', level: AnnotationLevel.stimme),
          _annot(id: 'o', level: AnnotationLevel.orchester),
        ],
        layerVisibility: const LayerVisibility(),
      );
      expect(() => _paintOnCanvas(painter), returnsNormally);
  ...
  group('DrawingPainter — aktiver Strich rendern', () {
    ...
    test('paint() mit einem Punkt → Dot (kein Fehler)', () {
      final painter = DrawingPainter(
        ...
        color: AppColors.annotationStimme,
        strokeWidth: 3.0,
        opacity: 1.0,
        tool: AnnotationTool.pencil,
      );
    ...
    test('paint() mit mehreren Punkten — Stift (kein Fehler)', () {
      final painter = DrawingPainter(
        ...
        color: AppColors.annotationOrchester,
        strokeWidth: 5.0,
        opacity: 1.0,
        tool: AnnotationTool.pencil,
      );
    ...
    testWidgets('Toolbar rendert alle Haupt-Werkzeuge', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          child: MaterialApp(
            home: Scaffold(
              body: AnnotationToolbar(stuckId: 'test-stuck'),
            ),
          ),
        ),
      );
