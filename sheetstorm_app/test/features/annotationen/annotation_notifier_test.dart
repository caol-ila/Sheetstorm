...
/// Creates a [ProviderContainer] scoped to the given stuckId.
/// Automatically disposed at end of test via [addTearDown].
(ProviderContainer, AnnotationNotifier) _setup(
  dynamic testContext, {
  String stuckId = 'test-stuck',
}) {
  final container = ProviderContainer();
  addTearDown(container.dispose);
  final notifier = container.read(annotationProvider(stuckId).notifier);
  return (container, notifier);
}

AnnotationState _state(ProviderContainer c, [String stuckId = 'test-stuck']) =>
    c.read(annotationProvider(stuckId));
...
void main() {
  ...
  group('addAnnotation — alle 3 Ebenen', () {
    test('Privat-Annotation wird hinzugefügt', () {
      ...
      n.addAnnotation(_annotation(level: AnnotationLevel.privat));
    ...
    test('Stimme-Annotation wird hinzugefügt', () {
      final (c, n) = _setup(null);
      n.addAnnotation(_annotation(level: AnnotationLevel.stimme));
      expect(_state(c).annotations.length, 1);
      expect(_state(c).annotations.first.level, AnnotationLevel.stimme);
    });

    test('Orchester-Annotation wird hinzugefügt', () {
      final (c, n) = _setup(null);
      n.addAnnotation(_annotation(level: AnnotationLevel.orchester));
      expect(_state(c).annotations.length, 1);
      expect(_state(c).annotations.first.level, AnnotationLevel.orchester);
    });

    test('Mehrere Annotationen akkumulieren sich', () {
      final (c, n) = _setup(null);
      n.addAnnotation(_annotation(id: 'a1', level: AnnotationLevel.privat));
      n.addAnnotation(_annotation(id: 'a2', level: AnnotationLevel.stimme));
      n.addAnnotation(_annotation(id: 'a3', level: AnnotationLevel.orchester));
  ...
  group('toggleLayerVisibility — Layer ein-/ausblenden', () {
    test('Privat ausblenden', () {
      final (c, n) = _setup(null);
      n.toggleLayerVisibility(AnnotationLevel.privat);
    ...
    test('Orchester ausblenden', () {
      final (c, n) = _setup(null);
      n.toggleLayerVisibility(AnnotationLevel.orchester);
    ...
    test('Ausgeblendete Ebene erscheint nicht in visibleAnnotations', () {
      final (c, n) = _setup(null);
      n.addAnnotation(_annotation(id: 'priv', level: AnnotationLevel.privat));
      n.addAnnotation(_annotation(id: 'orch', level: AnnotationLevel.orchester));
  ...
  group('visibleAnnotations — Seiten-Filterung', () {
    test('Nur Annotationen der aktuellen Seite sind sichtbar', () {
      final (c, n) = _setup(null);
    ...
    test('setPage wechselt sichtbare Annotationen', () {
      final (c, n) = _setup(null);
      n.addAnnotation(_annotation(id: 'p0', pageIndex: 0));
      n.addAnnotation(_annotation(id: 'p1', pageIndex: 1));
  ...
  group('annotationProvider — family-Scoping', () {
    test('Verschiedene stuckId-Scopes sind unabhängig', () {
      final container = ProviderContainer();
      addTearDown(container.dispose);
