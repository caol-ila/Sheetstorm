...
void main() {
  ...
  group('AnnotationLevel — Farb-Kodierung (Spec §1: Farbe = Reichweite)', () {
    test('Privat → Blau (#3B82F6)', () {
      expect(
        AnnotationLevel.privat.color.value,
    ...
    test('Stimme → Grün (#22C55E)', () {
      expect(
        AnnotationLevel.stimme.color.value,
        equals(AppColors.annotationStimme.value),
      );
      expect(
        AnnotationLevel.stimme.color.value,
    ...
    test('Orchester → Orange (#F97316)', () {
      expect(
        AnnotationLevel.orchester.color.value,
        equals(AppColors.annotationOrchester.value),
      );
      expect(
        AnnotationLevel.orchester.color.value,
    ...
    test('Labels sind korrekt deutsch', () {
      expect(AnnotationLevel.privat.label, 'Privat');
      expect(AnnotationLevel.stimme.label, 'Stimme');
      expect(AnnotationLevel.orchester.label, 'Orchester');
    });

    test('Beschreibungen decken Reichweite ab', () {
      expect(AnnotationLevel.privat.description, contains('mich'));
      expect(AnnotationLevel.stimme.description, contains('Stimme'));
      expect(AnnotationLevel.orchester.description, contains('Kapellenmitglieder'));
    });
  ...
  group('LayerVisibility — Layer-Toggle', () {
    test('Default: alle Ebenen sichtbar', () {
      ...
      expect(vis.isVisible(AnnotationLevel.privat), isTrue);
      expect(vis.isVisible(AnnotationLevel.stimme), isTrue);
      expect(vis.isVisible(AnnotationLevel.orchester), isTrue);
    });

    test('toggle(privat) schaltet nur Privat um', () {
      const vis = LayerVisibility();
      final toggled = vis.toggle(AnnotationLevel.privat);
    ...
    test('toggle(stimme) schaltet nur Stimme um', () {
      const vis = LayerVisibility();
      final toggled = vis.toggle(AnnotationLevel.stimme);
    ...
    test('toggle(orchester) schaltet nur Orchester um', () {
      const vis = LayerVisibility();
      final toggled = vis.toggle(AnnotationLevel.orchester);
      expect(toggled.privat, isTrue);
      expect(toggled.stimme, isTrue);
    ...
    test('isVisible respektiert false-Werte', () {
      ...
      expect(vis.isVisible(AnnotationLevel.privat), isFalse);
      expect(vis.isVisible(AnnotationLevel.stimme), isTrue);
      expect(vis.isVisible(AnnotationLevel.orchester), isFalse);
    });
  ...
  group('Annotation — Gleichheit per ID', () {
    test('Gleichheit basiert auf ID, nicht auf Inhalt', () {
      ...
      final b = a.copyWith(level: AnnotationLevel.orchester);
