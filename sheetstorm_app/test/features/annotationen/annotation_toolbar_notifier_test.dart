...
void main() {
  ...
  group('selectLevel — Ebenenauswahl', () {
    test('Standard-Ebene ist Privat', () {
      final (c, _) = _setup();
      expect(_state(c).activeLevel, AnnotationLevel.privat);
    });

    test('selectLevel(stimme) → activeLevel = stimme', () {
      final (c, n) = _setup();
      n.selectLevel(AnnotationLevel.stimme);
      expect(_state(c).activeLevel, AnnotationLevel.stimme);
    });

    test('selectLevel(orchester) → activeLevel = orchester', () {
      final (c, n) = _setup();
      n.selectLevel(AnnotationLevel.orchester);
      expect(_state(c).activeLevel, AnnotationLevel.orchester);
    });
  });
