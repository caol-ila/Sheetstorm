...
void main() {
  ...
  group('Berechtigungen — Dirigent vs. Nicht-Dirigent', () {
    ...
    testWidgets(
      'Dirigent: Tap auf Orchester gibt AnnotationLevel.orchestra zurück',
      (tester) async {
        AnnotationLevel? selected;
        ...
        expect(selected, AnnotationLevel.orchestra);
      },
    );
  });

  // ─── Jeder kann Privat-annotations erstellen ─────────────────────────────

  group('Privat-Ebene — für alle zugänglich', () {
    test('Notifier akzeptiert Privat-Annotation ohne Rollenbeschränkung', () {
      final (c, n) = _setup();
      n.addAnnotation(_annot(level: AnnotationLevel.private));
    ...
    testWidgets(
      'LevelPicker: Tap auf Privat gibt AnnotationLevel.private zurück',
      (tester) async {
        AnnotationLevel? selected;
                ...
                builder: (ctx) => ElevatedButton(
                  onPressed: () async {
                    selected = await showDialog<AnnotationLevel>(
                      context: ctx,
                      builder: (_) => const LevelPicker(
                        currentLevel: AnnotationLevel.voice,
                        isDirigent: false,
                      ),
                    );
                  },
        ...
        expect(selected, AnnotationLevel.private);
      },
    );
  });
  ...
  group('Stimme-Ebene — Layer-Toggle Sichtbarkeit', () {
    test('Stimme-annotations sind nur sichtbar wenn Stimme-Layer an', () {
      final (c, n) = _setup();
      n.addAnnotation(_annot(id: 'stimme-1', level: AnnotationLevel.voice));
    ...
    test('Stimme ausblenden lässt Privat und Orchester weiterhin sichtbar', () {
      final (c, n) = _setup();
      n.addAnnotation(_annot(id: 'priv', level: AnnotationLevel.private));
      n.addAnnotation(_annot(id: 'stim', level: AnnotationLevel.voice));
      n.addAnnotation(_annot(id: 'orch', level: AnnotationLevel.orchestra));
