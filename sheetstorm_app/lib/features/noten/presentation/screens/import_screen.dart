...
class _ImportScreenState extends ConsumerState<ImportScreen> {
  ImportZiel _ziel = ImportZiel.persoenlich;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: SnackBarAction(
              ...
              onPressed: () => ref.read(importProvider.notifier).fehlerZuruecksetzen(),
            ),
          ),
        );
      }
...
class _SelectionView extends StatelessWidget {
  ...
  final ValueChanged<ImportZiel> onZielChanged;
...
class _ZielAuswahl extends StatelessWidget {
  ...
  final ImportZiel ziel;
  final ValueChanged<ImportZiel> onChanged;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: AppSpacing.sm),
        SegmentedButton<ImportZiel>(
          segments: const [
            ButtonSegment(
              value: ImportZiel.persoenlich,
              label: Text('Persönliche Sammlung'),
              icon: Icon(Icons.person_outline),
            ),
            ButtonSegment(
              value: ImportZiel.kapelle,
              label: Text('Kapelle'),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.lg),
          // Overall progress
          LinearProgressIndicator(value: state.gesamtFortschritt),
          const SizedBox(height: AppSpacing.xs),
          Text(
            '${(state.gesamtFortschritt * 100).toStringAsFixed(0)} %',
...
class _FileProgressTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final isError = file.status == DateiUploadStatus.fehlgeschlagen;
    final isDone = file.status == DateiUploadStatus.hochgeladen;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          LinearProgressIndicator(value: state.fortschritt),
          if (state.seiten.isNotEmpty) ...[
            const SizedBox(height: AppSpacing.sm),
            Text(
              '${state.seiten.length} Seiten erkannt',
              style: theme.textTheme.bodyMedium,
              textAlign: TextAlign.center,
            ),
          ],
