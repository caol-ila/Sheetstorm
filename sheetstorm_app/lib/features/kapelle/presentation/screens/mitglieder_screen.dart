...
import 'package:sheetstorm/features/kapelle/application/mitglieder_notifier.dart';
...
import 'package:sheetstorm/features/kapelle/presentation/widgets/mitglied_list_tile.dart';

class MitgliederScreen extends ConsumerWidget {
  const MitgliederScreen({super.key, required this.kapelleId});

  final String kapelleId;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final membersState = ref.watch(mitgliederProvider(kapelleId));
    ...
    return Scaffold(
      ...
      floatingActionButton: isAdmin
          ? FloatingActionButton(
              onPressed: () =>
                  context.go(AppRoutes.kapelleEinladen(kapelleId: kapelleId)),
              tooltip: 'Einladen',
        ...
        error: (e, _) => Center(
          child: Column(
            ...
            children: [
              ...
              FilledButton(
                onPressed: () => ref
                    .read(mitgliederProvider(kapelleId).notifier)
                    .refresh(),
  ...
  Future<void> _showRoleEditSheet(
    ...
    final selectedRoles = <KapelleRolle>{...member.rollen};
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: AppSpacing.md),
                ...KapelleRolle.values.map((rolle) => CheckboxListTile(
                      value: selectedRoles.contains(rolle),
                      title: Text(rolle.label),
                      onChanged: (v) {
                        setSheetState(() {
                          if (v == true) {
                            selectedRoles.add(rolle);
                          } else {
                            selectedRoles.remove(rolle);
                          }
                        });
                      },
                    )),
                ...
                FilledButton(
                  onPressed: () async {
                    ...
                    final success = await ref
                        .read(
                            mitgliederProvider(kapelleId).notifier)
                        .updateRoles(
                          member.musikerId,
