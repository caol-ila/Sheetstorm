...
class ContextSettingsSheet extends StatelessWidget {
  ...
  Widget _buildFarbmodusTile() {
    return ListTile(
      leading: Icon(settings.farbmodus.icon, color: Colors.white70),
      title: const Text(
        'Anzeigemodus',
        style: TextStyle(color: Colors.white),
      ),
      trailing: SegmentedButton<Farbmodus>(
        segments: Farbmodus.values
            .map((m) => ButtonSegment<Farbmodus>(
                  value: m,
                  label: Text(
                    m.label,
            ...
            .toList(),
        selected: {settings.farbmodus},
        onSelectionChanged: (s) => onFarbmodusChanged(s.first),
        style: ButtonStyle(
      ...
      subtitle: Row(
        children: AnnotationLayer.values.map((layer) {
          final isActive = switch (layer) {
            AnnotationLayer.privat => settings.annotationPrivat,
            AnnotationLayer.stimme => settings.annotationStimme,
            AnnotationLayer.orchester => settings.annotationOrchester,
          };
  ...
  Widget _buildHelligkeitSlider() {
    return ListTile(
      ...
      subtitle: Slider(
        value: settings.helligkeit,
        min: 0.6,
        max: 1.0,
        divisions: 8,
        label: '${(settings.helligkeit * 100).round()}%',
        activeColor: AppColors.darkPrimary,
