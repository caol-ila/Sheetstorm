...
class AnnotationState {
  ...
  /// Alle annotations für das aktuelle Stück
  ...
  /// Sichtbare annotations für aktuelle Seite (gefiltert nach Sichtbarkeit)
  ...
  /// Alle annotations für eine bestimmte Seite
...
/// Provider uses `family` to scope per pieceId.
class AnnotationNotifier extends Notifier<AnnotationState> {
  ...
  /// Radieren: Lösche annotations, die den Radierer-Pfad schneiden
  void eraseAt({
    required double x,
    required double y,
    ...
    for (final a in state.visibleAnnotations) {
      // Nur eigene Ebene radierbar (UX-Spec: Radierer nur für eigene annotations)
  ...
  bool _hitTest(Annotation a, double x, double y, double radius) {
    // Für Punkt-basierte annotations: prüfe ob ein Punkt nah genug ist
    ...
    // Für BBox-basierte annotations (Text, Stempel)
    return x >= a.bbox.x &&
