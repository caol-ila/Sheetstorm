...
class _AnnotationLayerState extends ConsumerState<AnnotationLayer> {
  ...
  Widget build(BuildContext context) {
    final annotationState = ref.watch(annotationProvider(widget.pieceId));
  ...
  void _onPointerDown(PointerEvent event, bool isActive) {
    ...
    if (isStylusInput && !isActive) {
      ref.read(annotationProvider(widget.pieceId).notifier).enterAnnotationMode();
  ...
  void _onPointerMove(PointerEvent event, bool isActive) {
    ...
    if (tool == AnnotationTool.eraser) {
      ref.read(annotationProvider(widget.pieceId).notifier).eraseAt(
            x: relX,
            y: relY,
  ...
  void _onPointerUp(PointerEvent event, bool isActive) {
    ...
    if ((tool == AnnotationTool.pencil || tool == AnnotationTool.highlighter) &&
      ...
      ref.read(annotationProvider(widget.pieceId).notifier).commitStroke(
  ...
  void _exitAnnotationMode() {
    _activePoints.clear();
    ref.read(annotationProvider(widget.pieceId).notifier).exitAnnotationMode();
  ...
  void _showTextInput(double relX, double relY) {
    ...
    showDialog<String>(
      ...
      if (text != null && text.isNotEmpty) {
        ref.read(annotationProvider(widget.pieceId).notifier).addTextAnnotation(
              text: text,
              x: relX,
              y: relY,
              level: toolbarState.activeLevel,
  ...
  void _placeStamp(double relX, double relY) {
    ...
    ref.read(annotationProvider(widget.pieceId).notifier).addStampAnnotation(
          category: category,
          value: value,
          x: relX,
          y: relY,
